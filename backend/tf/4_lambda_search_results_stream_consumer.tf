...
locals {
  ...
  search_results_stream_consumer_lambda_file = "${var.out_dir}/searchResultsStreamConsumerHandler.zip"
}

resource "aws_lambda_function" "search_results_stream_consumer" {
  filename         = local.search_results_stream_consumer_lambda_file
